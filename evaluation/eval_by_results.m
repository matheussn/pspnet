...
for currDS = 1 : length(dsType) % iterating over different result folders
    fprintf(1, '%s\n', dsType(currDS));
    for currST = 1: length(stage)
        fprintf(1, '\t%s\n', stage(currST));
        for currTG = 1 : length(target) % iterating over the lesions classes

            path = fullfile(pwd, 'groundtruth/dysplasia/', target(currTG));
            gtDS = imageDatastore(path, 'IncludeSubfolders', true);
            ...
            saveDir = strcat(pwd, '/segmentation_eval/', dsType(currDS), '/', stage(currST), '/', target(currTG));
            mkdir(saveDir)
            ...
            for currImage = 1 : length(gtDS.Files)
                ...
                maskImName = char(strcat(pwd, '/segmentation_results/', dsType(currDS), '/', stage(currST), '/masks/', target(currTG), '/', name, '.tif'));
        ...
        allFeat = allFeat/4;
        allDev = allDev/4;
        pathName = strcat(pwd, '/segmentation_eval/', dsType(currDS), '/', stage(currST), '/', 'allFeat.csv');
